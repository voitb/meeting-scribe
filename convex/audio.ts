...
export const filterAudioAnalyses = query({
	...
	handler: async (ctx, args) => {
		...
		if (args.searchTerm && args.searchTerm.trim() !== "") {
			...
			filteredResults = totalResults.filter(
				(item) =>
...
export const storeAudioBlob = mutation({
	...
	handler: async (ctx, args) => {
		...
		if (args.fileSize > MAX_FILE_SIZE) {
			...
			throw new Error(
				`File size exceeds the 25MB limit. Current size: ${Math.round(args.fileSize / (1024 * 1024))}MB`
			);
		}

		console.log(
			`[Convex] Looking up analysis for url: ${args.audioId}, userId: ${userId}`
		);
		...
		console.log(
			`[Convex] Found existing analysis with ID: ${existingAnalysis._id}`
		);
...
export const getAudioBlob = query({
	...
	handler: async (ctx, args) => {
		...
		console.log(
			`[Convex] Looking for analysis with url: ${args.audioId}, userId: ${userId}`
		);
		...
		console.log(
			`[Convex] Audio blob size: ${analysis.audioBlob.byteLength} bytes`
		);
...
export const getAudioBlobPublic = query({
	...
	handler: async (ctx, args) => {
		console.log(
			`[Convex] getAudioBlobPublic called for audioId: ${args.audioId}`
		);
		...
		if (!analysis.audioBlob) {
			console.log(`[Convex] Analysis found but no audio blob is stored`);
			console.log(
				`[Convex] Analysis ID: ${analysis._id}, userId: ${analysis.userId}`
			);
			return null;
		}
...
export const updateAudioAnalysis = mutation({
	...
	handler: async (ctx, args) => {
		...
		console.log(
			`Looking for existing analysis with URL ${args.url} for user ${userId}`
		);
