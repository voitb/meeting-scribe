...
export function useAnalysisProcessor({
	...
	const saveAnalysisToConvex = useCallback(
	...
	const processAudio = useCallback(async () => {
		...
		try {
			...
			const transcriptionData = await transcriptionRes.json().catch(() => {
				throw new Error(
					"Failed to process server response. Transcription data format is invalid."
				);
			});

			if (!transcriptionData.transcription) {
				throw new Error(
					"No transcription found for this audio file. Please try again later."
				);
			}
			...
			const analysisResult = await response.json().catch(() => {
				throw new Error(
					"Failed to process server response. Analysis data format is invalid."
				);
			});
			...
			try {
				...
				toast.error(
					"Failed to save analysis in database, but you can still view the results"
				);
			}
