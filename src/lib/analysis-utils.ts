 ...
 */
export async function cleanAndParseAIResponse(
	responseText: string
	...
	try {
		...
		try {
			...
			if (!cleanedResponse.includes("{") || !cleanedResponse.includes("}")) {
				console.log(
					"Response lacks JSON structure. Returning text as summary."
				);
				return {
 ...
 */
export function splitTranscriptionIntoChunks(
	transcription: Transcription,
	maxSegmentsPerChunk: number = 50,
	maxTextLengthPerChunk: number = 15000
): Transcription[] {
	...
	console.log(
		`Transcription details: ${totalSegments} segments, ${totalTextLength} characters.`
	);

	// Check if chunking is needed based on segment count or text length
	if (
		totalSegments <= maxSegmentsPerChunk &&
		totalTextLength <= maxTextLengthPerChunk
	) {
	...
	console.log(
		`Chunking needed. Max segments/chunk: ${maxSegmentsPerChunk}, Max text length/chunk: ${maxTextLengthPerChunk}`
	);
	...
	while (currentSegmentIndex < totalSegments) {
		let chunkEndIndex = Math.min(
			currentSegmentIndex + maxSegmentsPerChunk,
			totalSegments
		);
		...
		for (let i = currentSegmentIndex; i < chunkEndIndex; i++) {
			...
			if (
				currentChunkTextLength + segmentTextLength > maxTextLengthPerChunk &&
				segmentCountInChunk > 5
			) {
		...
		const segmentsChunk = transcription.segments.slice(
			currentSegmentIndex,
			chunkEndIndex
		);
		...
		const chunkWords =
			transcription.words?.filter(
				(word) => word.start >= chunkStartTime && word.end <= chunkEndTime
			) || [];
		...
		console.log(
			`Created chunk ${chunks.length}: Segments ${currentSegmentIndex + 1}-${chunkEndIndex} (${segmentsChunk.length} segments, ${chunkText.length} chars)`
		);
...
export function mergeAnalysisResults(
	...
	if (!results || results.length === 0) {
		...
		// Return a default empty structure with an error
		return {
			title: originalTitle,
			...
			decisionsMade: [],
			videoChapters: [],
			error: "Error: No analysis results to merge.",
		};
	}

	if (results.length === 1) {
		console.log("Only one result, no merging needed.");
		// Check if video chapters are missing in the single result
		if (!results[0].videoChapters || results[0].videoChapters.length === 0) {
			console.error("Analysis is missing video chapters.");
			return {
				...results[0],
				error: "Error: Analysis is incomplete - try re-analyzing.",
			};
		}
		return results[0];
	}
	...
	const mergedChapters = results
		.flatMap((r) => r.videoChapters || [])
		.sort((a, b) => timeToSeconds(a.startTime) - timeToSeconds(b.startTime));

	// Check if there are no video chapters after merging
	if (mergedChapters.length === 0) {
		console.error("Merged analysis is missing video chapters.");
		return {
			title: originalTitle,
			summary: results
				.map((r) => r.summary)
			...
			decisionsMade: Array.from(
				new Set(results.flatMap((r) => r.decisionsMade || []))
			),
			videoChapters: [],
			error: "Error: Analysis is incomplete - missing video chapters.",
		};
	}
	...
	const merged: VideoAnalysisResult = {
		...
		videoChapters: mergedChapters, // Use the previously sorted chapters
	};
